                    ...
                    async with session.post(url=url, headers=headers, data=data, ssl=False) as response:
                        ...
                        if 'status' in play_game:
                            if play_game['status'] == 0:
                                self.print_timestamp(
                                    f"{Fore.BLUE + Style.BRIGHT}[ Game Started ][Subscribe Here- https://youtube.com/@hackerworldbd]{Style.RESET_ALL}"
                                    f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
...
if __name__ == '__main__':
    try:
        ...
        tomarket.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}[ Select an option ][Script By HACKER WORLD BD 😈]{Style.RESET_ALL}")
